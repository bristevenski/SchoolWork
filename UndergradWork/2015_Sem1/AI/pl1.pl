...
mammal_facts([has_hair, produces_milk]).
rodent_facts([has_hair, produces_milk, has_long_incisors]).

% Partial Classification facts:
fact(mammal, Facts) :- lists_equal(Facts, mammal_facts).
fact(rodent, Facts) :- lists_equal(Facts, rodent_facts).
...
is_mammal(X) :- member(X, [rodent, chipmunk]), !.